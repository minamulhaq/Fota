...
bootloader_cmd BootloaderCommandGetVersion = {B_CMD_GET_VERSION, 0x03, bcmd_default_handle, bcmd_get_version_response_buffer};
bootloader_cmd BootloaderCommandGetHelp = {B_CMD_GET_HELP, 0x00, bcmd_get_help_handle, bcmd_get_help_response_buffer};
bootloader_cmd BootloaderCommandGetCID = {B_CMD_GET_CID, 0x02, bcmd_default_handle, bcmd_get_cid_response_buffer};
bootloader_cmd BootloaderCommandGetRDPLevel = {B_CMD_GET_RDP_LVL, 0x01, bcmd_default_handle, bcmd_get_rdp_level_response_buffer};
bootloader_cmd BootloaderCommandJumpToAddress = {B_CMD_JMP_TO_ADDR, 0x00, bcmd_jump_to_addr_handle, bcmd_jump_to_addr_response_buffer};
bootloader_cmd BootloaderCommandEraseFlash = {B_CMD_ERASE_FLASH, 0x00, bcmd_erase_flash_handle, bcmd_erase_flash_response_buffer};

bootloader_cmd* cmd_table[] = {
    &BootloaderCommandGetVersion,
    &BootloaderCommandGetHelp,
    &BootloaderCommandGetCID,
    &BootloaderCommandGetRDPLevel,
    &BootloaderCommandJumpToAddress,
    &BootloaderCommandEraseFlash,
};
...
void bcmd_default_handle(bootloader_cmd* cmd, uint8_t* data) {
    ...
    CRC_VERIFICATION v = bootloader_verify_crc(data, command_packet_len - 4, host_crc);
...
uint8_t verify_address_space(uint32_t address) {
    if (address >= 0x0U && address < PERIPH_BASE) {
        return 0;
    }
    return 1;
}
...
void bcmd_erase_flash_handle(bootloader_cmd* cmd, uint8_t* data) {
    ...
    HAL_FLASH_Unlock();
    HAL_StatusTypeDef ret = HAL_FLASHEx_Erase(&flash_erase_handle, &sector_Error);
    HAL_FLASH_Lock();
    HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
    ret;
}
