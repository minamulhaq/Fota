...
class SerialMonitor:
    ...
    def send_command(self, cmd: Command) -> Optional[dict]:
        ...
        # Helper function to wait for and read one byte
        ...
        # Step 4c: Read Payload (only if length > 0)
        if payload_length > 0:
            ...
            for i in range(payload_length):
                ...
                # Print progress every 16 bytes or at end
        ...
        # Step 4d: Read CRC32 (4 bytes, little-endian)
        ...
        # Step 5: Complete packet received
        print(f"\n[RX] ✓ Complete packet received ({len(response_buffer)} bytes)")
        print(f"[RX] Raw data: {' '.join([f'{b:02X}' for b in response_buffer])}")

        # Step 6: Validate packet using command's receive_response_packet()
        validated_packet = cmd.receive_response_packet(bytes(response_buffer))

        if not validated_packet:
            print(f"\n[ERROR] Packet validation failed (CRC mismatch or malformed)")
            return None

        # Step 7: Parse response using command's handle_response()
